		...
		__init : function () {
			...
			this.get_container()
				...
				.delegate("a", "dblclick.jstree", $.proxy(function (e) {
						if (this._get_node(e.target).attr('rel') == 'disabled') { return false; }
						if(this._get_node(e.target).hasClass("jstree-checked")) { this.uncheck_node_all(e.target); }
						else { this.check_node_all(e.target); }
						if(this.data.ui) { this.save_selected(); }
						if(this.data.cookies) { this.save_cookie("select_node"); }
						e.preventDefault();
					}, this));
...
/*
				...
				obj.parentsUntil(".jstree", "li").each(function () {
					var $this = $(this);
					if(state) {
						// Если мы сняли галочку, но есть другие галочки у этого предка, то отмечаем его и его предков "неопределенными"
						if($this.children("ul").children(".jstree-checked, .jstree-undetermined").length) {
							$this.parentsUntil(".jstree", "li").andSelf().removeClass("jstree-checked jstree-unchecked").addClass("jstree-undetermined");
							return false;
						}
						// если мы сняли галочку и у предка галочек больше нету, но отмечаем как неотмеченного
						else {
							$this.removeClass("jstree-checked jstree-undetermined").addClass("jstree-unchecked");
						}
					}
					else {
						// Если мы поставили галочку, но у предка есть наследники неотмеченные галочками, то отмечаем предка и его предков "неопределенными"
						if($this.children("ul").children(".jstree-unchecked, .jstree-undetermined").length) {
							$this.parentsUntil(".jstree", "li").andSelf().removeClass("jstree-checked jstree-unchecked").addClass("jstree-undetermined");
							return false;
						}
						// Если мы поставили галочку и у предка все наследники с галочками, то отмечаем предка как отмеченного
						else {
							$this.removeClass("jstree-unchecked jstree-undetermined").addClass("jstree-checked");
						}
					}
...
//				return obj.find("> ul > .jstree-checked, .jstree-undetermined > ul > .jstree-checked");
			...
			get_unchecked : function (obj) { 
				obj = !obj || obj === -1 ? this.get_container() : this._get_node(obj);
				return obj.find(".jstree-unchecked");
