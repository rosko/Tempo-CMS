...
foreach ($units as $unit) {
    ...
    <div <?php if (!Yii::app()->user->isGuest) { ?>title="<?php echo call_user_func(array($className, 'name')); ?>"<?php } ?> id="cms-pageunit-<?=$unit->id?>" class="<?php if ($editArea) { ?>cms-pageunit <? } ?>pageunit cms-unit-<?=$unit->unit->type?>" rel="<?=$unit->unit->type?>" rev="<?=$unit->unit->id?>" content_id="<?=$unit->unit->content->id?>">
